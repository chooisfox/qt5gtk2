...
QGtkStylePrivate::QGtkStylePrivate() : QCommonStylePrivate(), filter(this)
{
...
void QGtkStylePrivate::cleanupGtkWidgets()
{
	...
	for (QHash<QHashableLatin1Literal, GtkWidget *>::const_iterator it = widgetMap->constBegin(); it != widgetMap->constEnd(); ++it)
		free(const_cast<char *>(it.key().data()));
}
