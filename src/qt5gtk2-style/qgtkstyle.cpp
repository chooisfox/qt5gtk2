...
static QPixmap qt_gtk_get_icon(const char *iconName, GtkIconSize size = GTK_ICON_SIZE_BUTTON)
{
	...
	GdkPixbuf  *icon	= gtk_icon_set_render_icon(iconSet, style, GTK_TEXT_DIR_LTR, GTK_STATE_NORMAL, size, NULL, "button");
...
{
	QColor dark;
	dark.setHsv(option->palette.button().color().hue(), qMin(255, (int)(option->palette.button().color().saturation() * 1.9)),
	...
	const QLine lines[4] = {
		QLine(tmp.left() + 2, tmp.top(), tmp.right() - 2, tmp.top()), QLine(tmp.left() + 2, tmp.bottom(), tmp.right() - 2, tmp.bottom()),
		QLine(tmp.left(), tmp.top() + 2, tmp.left(), tmp.bottom() - 2), QLine(tmp.right(), tmp.top() + 2, tmp.right(), tmp.bottom() - 2)};
...
static const char *const dock_widget_close_xpm[] = {"11 13 5 1",   "  c None",	  ". c #D5CFCB", "+ c #6C6A67", "@ c #6C6A67",
													"$ c #B5B0AC", "           ", " @@@@@@@@@ ", "@+       +@", "@ +@   @+ @",
													"@ @@@ @@@ @", "@  @@@@@  @", "@   @@@   @", "@  @@@@@  @", "@ @@@ @@@ @",
													"@ +@   @+ @", "@+       +@", " @@@@@@@@@ ", "           "};

static const char *const dock_widget_restore_xpm[] = {"11 13 5 1",	 "  c None",	". c #D5CFCB", "+ c #6C6A67", "@ c #6C6A67",
													  "# c #6C6A67", "           ", " @@@@@@@@@ ", "@+       +@", "@   #@@@# @",
													  "@   @   @ @", "@ #@@@# @ @", "@ @   @ @ @", "@ @   @@@ @", "@ @   @   @",
													  "@ #@@@@   @", "@+       +@", " @@@@@@@@@ ", "           "};

static const char *const qt_titlebar_context_help[] = {"10 10 3 1",	 "  c None",   "# c #000000", "+ c #444444", "  +####+  ",
													   " ###  ### ", " ##    ## ", "     +##+ ",  "    +##   ",	 "    ##    ",
													   "    ##    ", "          ", "    ##    ",  "    ##    "};

static const char *const qt_scrollbar_button_arrow_up[] = {"7 4 2 1", "   c None", "*  c #BFBFBF", "   *   ", "  ***  ", " ***** ", "*******"};

static const char *const qt_scrollbar_button_arrow_down[] = {"7 4 2 1", "   c None", "*  c #BFBFBF", "*******", " ***** ", "  ***  ", "   *   "};
...
*/
QGtkStyle::QGtkStyle() : QCommonStyle(*new QGtkStylePrivate)
{
	Q_D(QGtkStyle);
	d->init();
}
...
/*!
	...
	Constructs a QGtkStyle object.
*/
QGtkStyle::QGtkStyle(QGtkStylePrivate &dd) : QCommonStyle(dd)
{
	Q_D(QGtkStyle);
	d->init();
}
...
QPalette QGtkStyle::standardPalette() const
{
	...
	if (d->isThemeAvailable())
	{
		...
		style = gtk_rc_get_style_by_paths(gtk_settings_get_default(), "gtk-tooltips", "GtkWindow", gtk_window_get_type());
		if (style)
		{
...
void QGtkStyle::polish(QWidget *widget)
{
	...
	if (!d->isThemeAvailable())
		return;
	if (qobject_cast<QAbstractButton *>(widget) || qobject_cast<QToolButton *>(widget) || qobject_cast<QComboBox *>(widget) ||
		qobject_cast<QGroupBox *>(widget) || qobject_cast<QScrollBar *>(widget) || qobject_cast<QSlider *>(widget) ||
		qobject_cast<QAbstractSpinBox *>(widget) || qobject_cast<QSpinBox *>(widget) || qobject_cast<QHeaderView *>(widget))
...
int QGtkStyle::pixelMetric(PixelMetric metric, const QStyleOption *option, const QWidget *widget) const
{
	...
	switch (metric)
	{
		case PM_DefaultFrameWidth:
			...
			{
				if (GtkStyle *style =
						gtk_rc_get_style_by_paths(gtk_settings_get_default(), "*.GtkScrolledWindow", "*.GtkScrolledWindow", gtk_window_get_type()))
		...
		case PM_ScrollBarExtent: {
			...
			gtk_widget_style_get(hScrollbar, "trough-border", &trough_border, "slider-width", &sliderLength, NULL);
...
/*!
	\reimp
*/
void QGtkStyle::drawPrimitive(PrimitiveElement element, const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
	Q_D(const QGtkStyle);
	...
	switch (element)
	{
			...
			if (!QPixmapCache::find(pmKey, &pixmap))
			{
				...
				if (style)
					gtkPainter->paintShadow(d->gtkWidget("GtkFrame"), "viewport", pmRect,
											option->state & State_Enabled ? GTK_STATE_NORMAL : GTK_STATE_INSENSITIVE, shadow_type, style);
		...
		case PE_PanelStatusBar: {
			if (widget && widget->testAttribute(Qt::WA_SetPalette) && option->palette.resolve() & (1 << QPalette::Window))
			{
			...
			gtkPainter->paintShadow(gtkStatusbarFrame, "frame", option->rect, GTK_STATE_NORMAL, shadow_type,
									gtk_widget_get_style(gtkStatusbarFrame));
		}
		break;
		...
		case PE_IndicatorHeaderArrow:
			if (const QStyleOptionHeader *header = qstyleoption_cast<const QStyleOptionHeader *>(option))
			{
				...
				gtkPainter->paintArrow(gtkTreeHeader, "button", option->rect.adjusted(1, 1, -1, -1), type, state, GTK_SHADOW_NONE, false, style);
			}
			break;
		...
		case PE_IndicatorBranch:
			if (option->state & State_Children)
			{
				...
				gtkPainter->paintExpander(gtkTreeView, "treeview", rect, state, option->state & State_Open ? openState : closedState,
										  gtk_widget_get_style(gtkTreeView));
			}
			break;
		...
		case PE_PanelItemViewItem:
			if (const QStyleOptionViewItem *vopt = qstyleoption_cast<const QStyleOptionViewItem *>(option))
			{
				...
				if (vopt->backgroundBrush.style() != Qt::NoBrush || (resolve_mask & (1 << QPalette::Base)))
				{
		...
		case PE_IndicatorToolBarSeparator: {
			...
			if (option->state & State_Horizontal)
			{
				...
				gtkPainter->paintVline(gtkSeparator, "vseparator", rect, GTK_STATE_NORMAL, gtk_widget_get_style(gtkSeparator), 0, rect.height(), 0);
			}
			else
			{ // Draw vertical separator
				...
				painter->setPen(QPen(option->palette.window().color().darker(110)));
				gtkPainter->paintHline(gtkSeparator, "hseparator", rect, GTK_STATE_NORMAL, gtk_widget_get_style(gtkSeparator), 0, rect.width(), 0);
			}
		}
		break;
		...
		case PE_IndicatorToolBarHandle: {
			...
			gtkPainter->paintHandle(gtkToolbar, "toolbar", option->rect.adjusted(-1, -1, 0, 1), GTK_STATE_NORMAL, shadow_type,
									!(option->state & State_Horizontal) ? GTK_ORIENTATION_HORIZONTAL : GTK_ORIENTATION_VERTICAL,
									gtk_widget_get_style(gtkToolbar));
		}
		break;
		...
		case PE_IndicatorArrowRight: {
			...
			gtkPainter->paintArrow(gtkArrow, "button", arrowRect, type, state, shadow, false, gtk_widget_get_style(gtkArrow),
		...
		case PE_FrameMenu:
			...
			{
				GtkWidget *gtkMenubar = d->gtkWidget("GtkMenuBar");
				gtkPainter->paintBox(gtkMenubar, "toolbar", option->rect, GTK_STATE_NORMAL, GTK_SHADOW_OUT, style);
				gtkPainter->paintBox(gtkMenubar, "menu", option->rect, GTK_STATE_NORMAL, GTK_SHADOW_OUT, style);
			}
			break;
		...
		case PE_FrameLineEdit: {
			...
			QRect	 rect = option->rect;
			gtk_widget_style_get(gtkEntry, "interior-focus", &interior_focus, "focus-line-width", &focus_line_width, NULL);
			...
			gtkPainter->paintShadow(gtkEntry, "entry", rect, option->state & State_Enabled ? GTK_STATE_NORMAL : GTK_STATE_INSENSITIVE,
									GTK_SHADOW_IN, gtk_widget_get_style(gtkEntry), option->state & State_HasFocus ? QLS("focus") : QString());
			if (!interior_focus && option->state & State_HasFocus)
		...
		case PE_PanelLineEdit:
			if (const QStyleOptionFrame *panel = qstyleoption_cast<const QStyleOptionFrame *>(option))
			{
				...
				GtkStyle *gtkEntryStyle = gtk_widget_get_style(gtkEntry);
				QRect	  textRect		= option->rect.adjusted(gtkEntryStyle->xthickness, gtkEntryStyle->ythickness, -gtkEntryStyle->xthickness,
																-gtkEntryStyle->ythickness);

				if (widget && widget->testAttribute(Qt::WA_SetPalette) && resolve_mask & (1 << QPalette::Base)) // Palette overridden by user
					painter->fillRect(textRect, option->palette.base());
				else
					gtkPainter->paintFlatBox(gtkEntry, "entry_bg", textRect,
											 option->state & State_Enabled ? GTK_STATE_NORMAL : GTK_STATE_INSENSITIVE, GTK_SHADOW_NONE,
											 gtkEntryStyle);
			}
			break;
		...
		case PE_PanelButtonTool: {
			...
			gtk_widget_style_get(gtkButton, "focus-line-width", &focusWidth, "focus-padding", &focusPad, "interior-focus", &interiorFocus, NULL);
			...
			if (isDefault)
			{
				...
				gtkPainter->paintBox(gtkButton, "buttondefault", buttonRect, state, GTK_SHADOW_IN, style, isDefault ? QLS("d") : QString());
			}
			...
			GtkShadowType shadow = (option->state & State_Sunken || option->state & State_On) ? GTK_SHADOW_IN : GTK_SHADOW_OUT;

			gtkPainter->paintBox(gtkButton, "button", buttonRect, state, shadow, style, key);
			if (isDefault)
		...
		case PE_IndicatorCheckBox: {
			...
			gtkPainter->paintCheckbox(gtkCheckButton, checkRect, state, shadow, gtk_widget_get_style(gtkCheckButton), key);
			if (option->state & State_HasFocus)
				QGtkStylePrivate::gtkWidgetSetFocus(gtkCheckButton, false);
		}
		break;
			...
			if (const QStyleOptionTabBarBase *tbb = qstyleoption_cast<const QStyleOptionTabBarBase *>(option))
			{
				...
				switch (tbb->shape)
				{
					...
					case QTabBar::RoundedSouth:
						painter->drawLine(tbb->rect.left(), tbb->rect.bottom(), tabRect.right(), tabRect.bottom());
						break;
								   ...
								   const QWidget *widget) const
{
	...
	QColor		 grooveColor;
	QColor		 darkOutline;
	dark.setHsv(button.hue(), qMin(255, (int)(button.saturation() * 1.9)), qMin(255, (int)(button.value() * 0.7)));
	grooveColor.setHsv(button.hue(), qMin(255, (int)(button.saturation() * 2.6)), qMin(255, (int)(button.value() * 0.9)));
	darkOutline.setHsv(button.hue(), qMin(255, (int)(button.saturation() * 3.0)), qMin(255, (int)(button.value() * 0.6)));
	...
	switch (control)
	{
		...
		case CC_TitleBar:
			...
			if (const QStyleOptionTitleBar *titleBar = qstyleoption_cast<const QStyleOptionTitleBar *>(option))
			{
...

				{
					...
					QLinearGradient gradient(option->rect.center().x(), option->rect.top(), option->rect.center().x(), option->rect.bottom());
					...
					const QPoint points[5] = {QPoint(fullRect.left() + 4, fullRect.top() + 1), QPoint(fullRect.left() + 3, fullRect.top() + 1),
											  QPoint(fullRect.left() + 2, fullRect.top() + 2), QPoint(fullRect.left() + 1, fullRect.top() + 3),
					...
					const QPoint points2[5] = {QPoint(fullRect.right() - 3, fullRect.top() + 1), QPoint(fullRect.right() - 4, fullRect.top() + 1),
											   QPoint(fullRect.right() - 2, fullRect.top() + 2), QPoint(fullRect.right() - 1, fullRect.top() + 3),
					...
					!(titleBar->titleBarState & Qt::WindowMaximized))
				{
					...
					if (maxButtonRect.isValid())
					{
						...
						painter->drawLine(maxButtonIconRect.left() + 1, maxButtonIconRect.top() + 1, maxButtonIconRect.right() - 1,
										  maxButtonIconRect.top() + 1);
						painter->setPen(textAlphaColor);
						const QPoint points[4] = {maxButtonIconRect.topLeft(), maxButtonIconRect.topRight(), maxButtonIconRect.bottomLeft(),
				...
				if ((titleBar->subControls & SC_TitleBarCloseButton) && (titleBar->titleBarFlags & Qt::WindowSystemMenuHint))
				{
					...
					if (closeButtonRect.isValid())
					{
						...
						painter->setPen(textAlphaColor);
						const QLine lines[4] = {
							QLine(closeIconRect.left() + 1, closeIconRect.top(), closeIconRect.right(), closeIconRect.bottom() - 1),
							QLine(closeIconRect.left(), closeIconRect.top() + 1, closeIconRect.right() - 1, closeIconRect.bottom()),
							QLine(closeIconRect.right() - 1, closeIconRect.top(), closeIconRect.left(), closeIconRect.bottom() - 1),
							QLine(closeIconRect.right(), closeIconRect.top() + 1, closeIconRect.left() + 1, closeIconRect.bottom())};
						painter->drawLines(lines, 4);
				...
				if ((titleBar->subControls & SC_TitleBarNormalButton) &&
					(((titleBar->titleBarFlags & Qt::WindowMinimizeButtonHint) && (titleBar->titleBarState & Qt::WindowMinimized)) ||
					 ((titleBar->titleBarFlags & Qt::WindowMaximizeButtonHint) && (titleBar->titleBarState & Qt::WindowMaximized))))
				{
					...
					if (normalButtonRect.isValid())
					{
						...
						painter->setPen(textAlphaColor);
						const QPoint points[4] = {frontWindowRect.topLeft(), frontWindowRect.topRight(), frontWindowRect.bottomLeft(),
						...
						painter->setPen(textAlphaColor);
						const QPoint points2[4] = {backWindowRect.topLeft(), backWindowRect.topRight(), backWindowRect.bottomLeft(),
			...
			if (const QStyleOptionGroupBox *groupBox = qstyleoption_cast<const QStyleOptionGroupBox *>(option))
			{
				...
				if ((groupBox->subControls & QStyle::SC_GroupBoxLabel) && !groupBox->text.isEmpty())
				{
					...
					if (option->state & State_MouseOver)
					{
						...
						gtkPainter->paintFlatBox(gtkCheckButton, "checkbutton", bgRect.adjusted(0, 0, 0, -2), GTK_STATE_PRELIGHT,
												 GTK_SHADOW_ETCHED_OUT, gtk_widget_get_style(gtkCheckButton));
					}
		...
		case CC_ComboBox:
			...
			if (const QStyleOptionComboBox *comboBox = qstyleoption_cast<const QStyleOptionComboBox *>(option))
			{
				...
				bool  reverse		  = (option->direction == Qt::RightToLeft);
				QRect rect			  = option->rect;
				QRect arrowButtonRect = proxy()->subControlRect(CC_ComboBox, &comboBoxCopy, SC_ComboBoxArrow, widget);

				GtkShadowType				 shadow = (option->state & State_Sunken || option->state & State_On) ? GTK_SHADOW_IN : GTK_SHADOW_OUT;
				const QHashableLatin1Literal comboBoxPath =
					comboBox->editable ? QHashableLatin1Literal("GtkComboBoxEntry") : QHashableLatin1Literal("GtkComboBox");
				...
				if (gtkToggleButton && (appears_as_list || comboBox->editable))
				{
					...
					if (comboBox->editable || appears_as_list)
					{
						...
						uint	  resolve_mask	= option->palette.resolve();
						GtkStyle *gtkEntryStyle = gtk_widget_get_style(gtkEntry);
						QRect	  contentRect = frameRect.adjusted(gtkEntryStyle->xthickness, gtkEntryStyle->ythickness, -gtkEntryStyle->xthickness,
																   -gtkEntryStyle->ythickness);
						...
						else
						{
							gtkPainter->paintFlatBox(gtkEntry, "entry_bg", contentRect,
													 option->state & State_Enabled ? GTK_STATE_NORMAL : GTK_STATE_INSENSITIVE, GTK_SHADOW_NONE,
													 gtkEntryStyle, entryPath.toString() + QString::number(focus));
						}

						gtkPainter->paintShadow(gtkEntry, comboBox->editable ? "entry" : "frame", frameRect, frameState, GTK_SHADOW_IN,
												gtkEntryStyle,
												entryPath.toString() + QString::number(focus) + QString::number(comboBox->editable) +
					...
					gtkPainter->paintBox(gtkToggleButton, "button", arrowButtonRect, buttonState, shadow, gtk_widget_get_style(gtkToggleButton),
										 buttonPath.toString() + QString::number(focus) + QString::number(option->direction));
					if (focus)
				...
				else
				{
					...
					if (focus) // Clearlooks actually check the widget for the default state
						QGtkStylePrivate::gtkWidgetSetFocus(gtkToggleButton, true);
					gtkPainter->paintBox(gtkToggleButton, "button", buttonRect, state, shadow, gtkToggleButtonStyle,
					...
					if (GtkWidget *gtkVSeparator = d->gtkWidget(vSeparatorPath))
					{
						...
						gtkPainter->paintVline(gtkVSeparator, "vseparator", vLineRect, state, gtk_widget_get_style(gtkVSeparator), 0,
											   vLineRect.height(), 0, vSeparatorPath.toString());
						...
						if (focus && ((option->state & State_KeyboardFocusChange) || styleHint(SH_UnderlineShortcut, option, widget)))
							gtkPainter->paintFocus(gtkToggleButton, "button", option->rect.adjusted(xt, yt, -xt, -yt),
												   option->state & State_Sunken ? GTK_STATE_ACTIVE : GTK_STATE_NORMAL, gtkToggleButtonStyle);
					}
				}
				...
				if (comboBox->subControls & SC_ComboBoxArrow)
				{
					...
					{
						gtkPainter->setClipRect(option->rect);
						gtkPainter->paintArrow(gtkArrow, "arrow", arrowRect, GTK_ARROW_DOWN, state, GTK_SHADOW_NONE, true, style,
											   arrowPath.toString() + QString::number(option->direction));
					}
				}
		...
		case CC_ToolButton:
			if (const QStyleOptionToolButton *toolbutton = qstyleoption_cast<const QStyleOptionToolButton *>(option))
			{
				...
				bool drawMenuArrow =
					toolbutton->features & QStyleOptionToolButton::HasMenu && !(toolbutton->features & QStyleOptionToolButton::MenuButtonPopup);
				...
				if (option->state & State_Enabled && option->state & State_MouseOver && !(widget && widget->testAttribute(Qt::WA_SetPalette)))
				{
				...
				label.rect = button.adjusted(style->xthickness, style->ythickness, -style->xthickness - popupArrowSize, -style->ythickness);
		...
		case CC_ScrollBar:
			if (const QStyleOptionSlider *scrollBar = qstyleoption_cast<const QStyleOptionSlider *>(option))
			{
				...
				if (!gtk_check_version(2, 10, 0))
				{
					gtk_widget_style_get((GtkWidget *)(scrollbarWidget), "trough-border", &trough_border, "trough-side-details",
										 &trough_side_details, "trough-under-steppers", &trough_under_steppers,
				...
				if (trough_under_steppers)
				{
					...
					scrollBarSlider.adjust(horizontal ? -trough_border : 0, horizontal ? 0 : -trough_border, horizontal ? trough_border : 0,
										   horizontal ? 0 : trough_border);
				}
				...
				if (scrollBar->subControls & SC_ScrollBarSlider)
				{
					...
					if (!(option->state & State_Enabled))
						state = GTK_STATE_INSENSITIVE;
					else if (activate_slider && option->state & State_Sunken && (scrollBar->activeSubControls & SC_ScrollBarSlider))
						state = GTK_STATE_ACTIVE;
					...
					gtkPainter->paintSlider(scrollbarWidget, "slider", scrollBarSlider, state, shadow, style,
											...
											QString(QLS("%0%1")).arg(fakePos).arg(maximum));
				}
				...
				if (scrollBar->subControls & SC_ScrollBarAddLine)
				{
					...
					gtkPainter->paintBox(scrollbarWidget, horizontal ? "hscrollbar" : "vscrollbar", scrollBarAddLine, state, shadow, style,
										 QLS("add"));

					gtkPainter->paintArrow(scrollbarWidget, horizontal ? "hscrollbar" : "vscrollbar", scrollBarAddLine.adjusted(4, 4, -4, -4),
										   horizontal ? (reverse ? GTK_ARROW_LEFT : GTK_ARROW_RIGHT) : GTK_ARROW_DOWN, state, GTK_SHADOW_NONE,
										   false, style);
				}
				...
				if (scrollBar->subControls & SC_ScrollBarSubLine)
				{
					...
					gtkPainter->paintBox(scrollbarWidget, horizontal ? "hscrollbar" : "vscrollbar", scrollBarSubLine, state, shadow, style,
										 QLS("sub"));

					gtkPainter->paintArrow(scrollbarWidget, horizontal ? "hscrollbar" : "vscrollbar", scrollBarSubLine.adjusted(4, 4, -4, -4),
										   horizontal ? (reverse ? GTK_ARROW_RIGHT : GTK_ARROW_LEFT) : GTK_ARROW_UP, state, GTK_SHADOW_NONE, false,
										   style);
				}
			}
			break;
		...
		case CC_SpinBox:
			if (const QStyleOptionSpinBox *spinBox = qstyleoption_cast<const QStyleOptionSpinBox *>(option))
			{
				...
				GtkWidget *gtkSpinButton =
					spinBox->buttonSymbols == QAbstractSpinBox::NoButtons ? d->gtkWidget("GtkEntry") : d->gtkWidget("GtkSpinButton");
				...
				if (spinBox->frame)
				{
					...
					if (resolve_mask & (1 << QPalette::Base)) // Palette overridden by user
						...
						gtkPainter->paintFlatBox(gtkSpinButton, "entry_bg",
												 editArea.adjusted(style->xthickness, style->ythickness, -style->xthickness, -style->ythickness),
												 option->state & State_Enabled ? GTK_STATE_NORMAL : GTK_STATE_INSENSITIVE, GTK_SHADOW_NONE, style,
												 key);
				...
				else if (spinBox->buttonSymbols == QAbstractSpinBox::UpDownArrows)
				{
					...
					gtkPainter->paintArrow(gtkSpinButton, "spinbutton", arrowRect, GTK_ARROW_UP, state, GTK_SHADOW_NONE, false, style);
					...
					gtkPainter->paintArrow(gtkSpinButton, "spinbutton", arrowRect, GTK_ARROW_DOWN, state, GTK_SHADOW_NONE, false, style);
				}
			}
			break;
		...
		case CC_Slider:
			if (const QStyleOptionSlider *slider = qstyleoption_cast<const QStyleOptionSlider *>(option))
			{
				...
				gtk_widget_set_direction(hScaleWidget, slider->upsideDown ? GTK_TEXT_DIR_RTL : GTK_TEXT_DIR_LTR);
				...
				if ((option->subControls & SC_SliderGroove) && groove.isValid())
				{
					...
					if (adjustment)
					{
						gtk_adjustment_configure(adjustment, slider->sliderPosition, slider->minimum, slider->maximum, slider->singleStep,
												 slider->singleStep, slider->pageStep);
					}
					else
					{
						adjustment = (GtkAdjustment *)gtk_adjustment_new(slider->sliderPosition, slider->minimum, slider->maximum,
																		 slider->singleStep, slider->singleStep, slider->pageStep);
						gtk_range_set_adjustment(range, adjustment);
					}
					...
					QRect		 grooveRect =
						option->rect.adjusted(focusFrameMargin, outerSize + focusFrameMargin, -focusFrameMargin, -outerSize - focusFrameMargin);
					...
					{
						gtkPainter->paintBox(scaleWidget, "trough", grooveRect, state, GTK_SHADOW_IN, style,
											 QString(QLS("p%0")).arg(slider->sliderPosition));
					}
					else
					{
						...
						gtkPainter->paintBox(scaleWidget, "trough-upper", upperGroove, state, GTK_SHADOW_IN, style,
											 QString(QLS("p%0")).arg(slider->sliderPosition));
						gtkPainter->paintBox(scaleWidget, "trough-lower", lowerGroove, state, GTK_SHADOW_IN, style,
											 QString(QLS("p%0")).arg(slider->sliderPosition));
					}
				}
...
/*!
	\reimp
*/
void QGtkStyle::drawControl(ControlElement element, const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
	Q_D(const QGtkStyle);
	...
	switch (element)
	{
		case CE_ProgressBarLabel:
			if (const QStyleOptionProgressBar *bar = qstyleoption_cast<const QStyleOptionProgressBar *>(option))
			{
				...
				const int progressIndicatorPos =
					(bar->progress - qreal(bar->minimum)) * rect.width() / qMax(qreal(1.0), qreal(bar->maximum) - bar->minimum);
				...
				bool flip =
					(!vertical && (((bar->direction == Qt::RightToLeft) && !inverted) || ((bar->direction == Qt::LeftToRight) && inverted)));
		...
		case CE_PushButtonLabel:
			if (const QStyleOptionButton *button = qstyleoption_cast<const QStyleOptionButton *>(option))
			{
				...
				if (option->state & State_Sunken)
					buttonShift =
						QPoint(pixelMetric(PM_ButtonShiftHorizontal, option, widget), proxy()->pixelMetric(PM_ButtonShiftVertical, option, widget));
				...
				int		   labelState = GTK_STATE_INSENSITIVE;
				if (option->state & State_Enabled)
					labelState = (option->state & State_MouseOver && !(option->state & State_Sunken)) ? GTK_STATE_PRELIGHT : GTK_STATE_NORMAL;

				GdkColor gdkText   = gtk_widget_get_style(gtkButton)->fg[labelState];
				QColor	 textColor = QColor(gdkText.red >> 8, gdkText.green >> 8, gdkText.blue >> 8);
				pal.setBrush(QPalette::ButtonText, textColor);
				proxy()->drawItemText(painter, ir, tf, pal, (button->state & State_Enabled), button->text, QPalette::ButtonText);
			}
			break;
		...
		case CE_CheckBox:
			if (const QStyleOptionButton *btn = qstyleoption_cast<const QStyleOptionButton *>(option))
			{
				...
				if (option->state & State_MouseOver)
				{
					gtkPainter->paintFlatBox(gtkRadioButton, "checkbutton", option->rect, GTK_STATE_PRELIGHT, GTK_SHADOW_ETCHED_OUT,
											 gtk_widget_get_style(gtkRadioButton));
				}
		...
		case CE_DockWidgetTitle:
			...
			if (const QStyleOptionDockWidget *dwOpt = qstyleoption_cast<const QStyleOptionDockWidget *>(option))
			{
				...
				if (verticalTitleBar)
				{
					...
					titleRect = QRect(r.left() + rect.bottom() - titleRect.bottom(), r.top() + titleRect.left() - rect.left(), titleRect.height(),
									  titleRect.width());
				...
				if (!dwOpt->title.isEmpty())
				{
					...
					proxy()->drawItemText(painter, titleRect, Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic, dwOpt->palette,
		...
		case CE_HeaderSection:
			...
			if (const QStyleOptionHeader *header = qstyleoption_cast<const QStyleOptionHeader *>(option))
			{
				...
				gtkPainter->paintBox(gtkTreeHeader, "button", option->rect.adjusted(-1, 0, 0, 0), state, shadow,
									 gtk_widget_get_style(gtkTreeHeader));
			}
		...
		case CE_SizeGrip: {
			...
			gtkPainter->paintResizeGrip(gtkStatusbar, "statusbar", gripRect, GTK_STATE_NORMAL, GTK_SHADOW_OUT,
										option->direction == Qt::RightToLeft ? GDK_WINDOW_EDGE_SOUTH_WEST : GDK_WINDOW_EDGE_SOUTH_EAST,
										gtkStatusbarStyle);
		}
		break;
		...
		case CE_MenuBarEmptyArea: {
			...
			{ // See CE_MenuBarItem
				...
				gtkPainter->paintBox(gtkMenubar, "menubar", menuBarRect, GTK_STATE_NORMAL, shadow_type, gtk_widget_get_style(gtkMenubar));
				pmPainter.end();
		...
		case CE_MenuBarItem:
			...
			if (const QStyleOptionMenuItem *mbi = qstyleoption_cast<const QStyleOptionMenuItem *>(option))
			{
				...
				if (act)
				{
					...
					gtkPainter->paintBox(gtkMenubarItem, "menuitem", option->rect.adjusted(0, 0, 0, 3), GTK_STATE_PRELIGHT, shadowType, style);
					// draw text
		...
		case CE_Splitter: {
			...
			gtkPainter->paintHandle(gtkWindow, "splitter", option->rect, qt_gtk_state(option), GTK_SHADOW_NONE,
									!(option->state & State_Horizontal) ? GTK_ORIENTATION_HORIZONTAL : GTK_ORIENTATION_VERTICAL, style);
		}
		break;
		...
		case CE_ToolBar:
			if (const QStyleOptionToolBar *toolbar = qstyleoption_cast<const QStyleOptionToolBar *>(option))
			{
				...
				GtkShadowType shadow_type = GTK_SHADOW_NONE;
				gtk_widget_style_get(gtkToolbar, "shadow-type", &shadow_type, NULL);
				gtkPainter->paintBox(gtkToolbar, "toolbar", rect, GTK_STATE_NORMAL, shadow_type, gtk_widget_get_style(gtkToolbar));
			}
			break;
		...
		case CE_MenuItem:
			...
			if (const QStyleOptionMenuItem *menuItem = qstyleoption_cast<const QStyleOptionMenuItem *>(option))
			{
				...
				GtkWidget *gtkMenuItem		  = menuItem->checked ? d->gtkWidget("GtkMenu.GtkCheckMenuItem") : d->gtkWidget("GtkMenu.GtkMenuItem");
				...
				if (menuItem->menuItemType == QStyleOptionMenuItem::Separator)
				{
					...
					if (!gtk_check_version(2, 10, 0))
					{
						gtk_widget_style_get(gtkMenuSeparator, "wide-separators", &wide_separators, "separator-height", &separator_height,
					...
					if (wide_separators)
						gtkPainter->paintBox(gtkMenuSeparator, "hseparator", separatorRect, GTK_STATE_NORMAL, GTK_SHADOW_NONE,
											 gtkMenuSeparatorStyle);
					else
						gtkPainter->paintHline(gtkMenuSeparator, "hseparator", separatorRect, GTK_STATE_NORMAL, gtkMenuSeparatorStyle, 0,
											   option->rect.right() - 1, 1);
					painter->restore();
					break;
				}
				...
				if (!ignoreCheckMark)
				{
					...
					if (checkable && menuItem->icon.isNull())
					{
						...
						if (menuItem->checkType & QStyleOptionMenuItem::Exclusive)
						{
							...
							gtkPainter->paintOption(gtkMenuItem, checkRect.translated(-spacing, -spacing), state, shadow, style, QLS("option"));
						...
						else
						{
							...
							if (menuItem->icon.isNull())
							{
								...
								gtkPainter->setClipRect(checkRect.adjusted(-spacing, -spacing, -spacing, -spacing));
								gtkPainter->paintCheckbox(gtkMenuItem, checkRect.translated(-spacing, -spacing), state, shadow, style,
														  QLS("check"));
								gtkPainter->setClipRect(QRect());
							}
						}
				...
				QRect						vCheckRect = visualRect(opt->direction, menuitem->rect,
																	QRect(menuitem->rect.x() + 3, menuitem->rect.y(), checkcol, menuitem->rect.height()));
				...
				{ // draw sub menu arrow
					...
					gtkPainter->paintArrow(gtkMenuItem, "menuitem", vSubMenuRect,
										   option->direction == Qt::RightToLeft ? GTK_ARROW_LEFT : GTK_ARROW_RIGHT, state, shadowType, false,
										   style);
				}
			}
			painter->restore();
		...
		case CE_PushButton:
			if (const QStyleOptionButton *btn = qstyleoption_cast<const QStyleOptionButton *>(option))
			{
				...
				if (btn->features & QStyleOptionButton::Flat && btn->state & State_HasFocus)
					...
					gtkPainter->paintFocus(gtkButton, "button", option->rect.adjusted(xt, yt, -xt, -yt),
			...
			if (const QStyleOptionTab *tab = qstyleoption_cast<const QStyleOptionTab *>(option))
			{
				...
				switch (tab->shape)
				{
					...
					case QTabBar::RoundedSouth:
						...
						gtkPainter->paintExtention(gtkNotebook, "tab", rect.adjusted(0, 1, 0, 0), state, shadow, GTK_POS_TOP, style);
						break;
...
/*!
  \reimp
*/
QRect QGtkStyle::subControlRect(ComplexControl control, const QStyleOptionComplex *option, SubControl subControl, const QWidget *widget) const
{
	Q_D(const QGtkStyle);
	...
	switch (control)
	{
				...
				switch (sc)
				{
					...
					case SC_TitleBarCloseButton:
						if (tb->titleBarFlags & Qt::WindowSystemMenuHint)
							...
							break;
						ret.setRect(tb->rect.right() - indent - offset, tb->rect.top() + controlTopMargin, controlHeight, controlHeight);
						break;
					case SC_TitleBarSysMenu:
						if (tb->titleBarFlags & Qt::WindowSystemMenuHint)
						{
							ret.setRect(tb->rect.left() + controlWidthMargin + indent, tb->rect.top() + controlTopMargin, controlHeight,
										controlHeight);
						}
						break;
					default:
		...
		case CC_ComboBox:
			if (const QStyleOptionComboBox *box = qstyleoption_cast<const QStyleOptionComboBox *>(option))
			{
				...
				GtkWidget *gtkCombo = box->editable ? d->gtkWidget("GtkComboBoxEntry") : d->gtkWidget("GtkComboBox");
				...
				QRect buttonRect(option->rect.left() + allocation.x, option->rect.top() + allocation.y, allocation.width, allocation.height);
...
/*!
  \reimp
*/
QSize QGtkStyle::sizeFromContents(ContentsType type, const QStyleOption *option, const QSize &size, const QWidget *widget) const
{
	Q_D(const QGtkStyle);
	...
	switch (type)
	{
		...
		case CT_PushButton:
			if (const QStyleOptionButton *btn = qstyleoption_cast<const QStyleOptionButton *>(option))
			{
				...
				gtk_widget_style_get(gtkButtonBox, "child-min-width", &minWidth, "child-min-height", &minHeight, NULL);
...
/*! \reimp */
QPixmap QGtkStyle::standardPixmap(StandardPixmap sp, const QStyleOption *option, const QWidget *widget) const
{
	Q_D(const QGtkStyle);
...
/*!
  \reimp
*/
void QGtkStyle::drawItemPixmap(QPainter *painter, const QRect &rect, int alignment, const QPixmap &pixmap) const
{
...
/*!
  \reimp
*/
QStyle::SubControl QGtkStyle::hitTestComplexControl(ComplexControl cc, const QStyleOptionComplex *opt, const QPoint &pt, const QWidget *w) const
{
...
/*!
  \reimp
*/
QPixmap QGtkStyle::generatedIconPixmap(QIcon::Mode iconMode, const QPixmap &pixmap, const QStyleOption *opt) const
{
