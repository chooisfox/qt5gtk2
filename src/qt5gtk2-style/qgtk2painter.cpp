...
QPixmap QGtk2Painter::renderTheme(uchar *bdata, uchar *wdata, const QRect &rect) const
{
	...
	for (int index = 0; index < bytecount; index += 4)
	{
		...
		if (m_alpha)
		{
			int alphaval			= qMax(bdata[index + GTK_BLUE] - wdata[index + GTK_BLUE], bdata[index + GTK_GREEN] - wdata[index + GTK_GREEN]);
	...
	QImage converted((const uchar *)bdata, rect.width(), rect.height(), m_alpha ? QImage::Format_ARGB32_Premultiplied : QImage::Format_RGB32);
...
#define DRAW_TO_CACHE(draw_func)                                                                                                                   \
	...
	{                                                                                                                                              \
		GdkPixmap *pixmap = gdk_pixmap_new((GdkDrawable *)(m_window->window), rect.width(), rect.height(), -1);                                    \
		if (!pixmap)                                                                                                                               \
			return;                                                                                                                                \
		style = gtk_style_attach(style, m_window->window);                                                                                         \
		gdk_draw_rectangle(pixmap, m_alpha ? style->black_gc : *style->bg_gc, true, 0, 0, rect.width(), rect.height());                            \
		draw_func;                                                                                                                                 \
		GdkPixbuf *imgb = gdk_pixbuf_new(GDK_COLORSPACE_RGB, true, 8, rect.width(), rect.height());                                                \
		if (!imgb)                                                                                                                                 \
			return;                                                                                                                                \
		imgb		 = gdk_pixbuf_get_from_drawable(imgb, pixmap, NULL, 0, 0, 0, 0, rect.width(), rect.height());                                  \
		uchar *bdata = (uchar *)gdk_pixbuf_get_pixels(imgb);                                                                                       \
		if (m_alpha)                                                                                                                               \
		{                                                                                                                                          \
			gdk_draw_rectangle(pixmap, style->white_gc, true, 0, 0, rect.width(), rect.height());                                                  \
			draw_func;                                                                                                                             \
			GdkPixbuf *imgw = gdk_pixbuf_new(GDK_COLORSPACE_RGB, true, 8, rect.width(), rect.height());                                            \
			if (!imgw)                                                                                                                             \
				return;                                                                                                                            \
			imgw		 = gdk_pixbuf_get_from_drawable(imgw, pixmap, NULL, 0, 0, 0, 0, rect.width(), rect.height());                              \
...
void QGtk2Painter::paintBoxGap(GtkWidget	  *gtkWidget,
							   const gchar	  *part,
							   ...
							   GtkStyle		  *style)
{
	...
	QString pixmapName =
		uniqueName(QLS(part), state, shadow, rect.size(), gtkWidget) % HexString<uchar>(gap_side) % HexString<gint>(width) % HexString<gint>(x);

	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_box_gap(style, pixmap, state, shadow, NULL, gtkWidget, (const gchar *)part, 0, 0, rect.width(), rect.height(),
										gap_side, x, width));
		if (m_usePixmapCache)
	...
	if (rect.size() != paintRect.size())
	{
		...
		// top part
		m_painter->drawPixmap(QRect(paintRect.left(), paintRect.top(), paintRect.width(), border), cache, QRect(0, 0, size.width(), border));
		...
		scanLinePainter.end();
		m_painter->drawTiledPixmap(QRect(paintRect.left(), paintRect.top() + border, paintRect.width(), paintRect.height() - 2 * border), tilePart);

		// bottom part
		m_painter->drawPixmap(QRect(paintRect.left(), paintRect.top() + paintRect.height() - border, paintRect.width(), border), cache,
...
void QGtk2Painter::paintBox(GtkWidget	  *gtkWidget,
							const gchar	  *part,
							const QRect	  &paintRect,
							GtkStateType   state,
							GtkShadowType  shadow,
							...
							const QString &pmKey)
{
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_box(style, pixmap, state, shadow, NULL, gtkWidget, part, 0, 0, rect.width(), rect.height()));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
	if (rect.size() != paintRect.size())
...
void QGtk2Painter::paintHline(GtkWidget		*gtkWidget,
							  const gchar	*part,
							  const QRect	&rect,
							  GtkStateType	 state,
							  GtkStyle		*style,
							  ...
							  const QString &pmKey)
{
	...
	QPixmap cache;
	QString pixmapName =
		uniqueName(QLS(part), state, GTK_SHADOW_NONE, rect.size(), gtkWidget) % HexString<int>(x1) % HexString<int>(x2) % HexString<int>(y) % pmKey;
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_hline(style, pixmap, state, NULL, gtkWidget, part, x1, x2, y));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
...
void QGtk2Painter::paintVline(GtkWidget		*gtkWidget,
							  const gchar	*part,
							  const QRect	&rect,
							  GtkStateType	 state,
							  GtkStyle		*style,
							  ...
							  const QString &pmKey)
{
	...
	QPixmap cache;
	QString pixmapName =
		uniqueName(QLS(part), state, GTK_SHADOW_NONE, rect.size(), gtkWidget) % HexString<int>(y1) % HexString<int>(y2) % HexString<int>(x) % pmKey;

	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_vline(style, pixmap, state, NULL, gtkWidget, part, y1, y2, x));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
	m_painter->drawPixmap(rect.topLeft(), cache);
								 ...
								 const QString	 &pmKey)
{
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_expander(style, pixmap, state, NULL, gtkWidget, part, rect.width() / 2, rect.height() / 2, expander_state));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
...
void QGtk2Painter::paintFocus(GtkWidget *gtkWidget, const gchar *part, const QRect &rect, GtkStateType state, GtkStyle *style, const QString &pmKey)
{
	if (!rect.isValid())
		return;
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_focus(style, pixmap, state, NULL, gtkWidget, part, 0, 0, rect.width(), rect.height()));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
...
void QGtk2Painter::paintResizeGrip(GtkWidget	 *gtkWidget,
								   const gchar	 *part,
								   const QRect	 &rect,
								   GtkStateType	  state,
								   GtkShadowType  shadow,
								   ...
								   const QString &pmKey)
{
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_resize_grip(style, pixmap, state, NULL, gtkWidget, part, edge, 0, 0, rect.width(), rect.height()));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
...
void QGtk2Painter::paintArrow(GtkWidget		*gtkWidget,
							  const gchar	*part,
							  ...
							  const QString &pmKey)
{
	...
	QPixmap cache;
	QString pixmapName = uniqueName(QLS(part), state, shadow, rect.size()) % HexString<uchar>(arrow_type) % pmKey;
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_arrow(style, pixmap, state, shadow, &gtkCliprect, gtkWidget, part, arrow_type, fill, xOffset, yOffset,
...
void QGtk2Painter::paintHandle(GtkWidget	 *gtkWidget,
							   const gchar	 *part,
							   const QRect	 &rect,
							   GtkStateType	  state,
							   GtkShadowType  shadow,
							   ...
							   GtkStyle		 *style)
{
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_handle(style, pixmap, state, shadow, NULL, gtkWidget, part, 0, 0, rect.width(), rect.height(), orientation));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
	m_painter->drawPixmap(rect.topLeft(), cache);
...
void QGtk2Painter::paintSlider(GtkWidget	 *gtkWidget,
							   const gchar	 *part,
							   const QRect	 &rect,
							   GtkStateType	  state,
							   GtkShadowType  shadow,
							   ...
							   const QString &pmKey)
{
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_slider(style, pixmap, state, shadow, NULL, gtkWidget, part, 0, 0, rect.width(), rect.height(), orientation));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
	m_painter->drawPixmap(rect.topLeft(), cache);
...
void QGtk2Painter::paintShadow(GtkWidget	 *gtkWidget,
							   const gchar	 *part,
							   const QRect	 &rect,
							   GtkStateType	  state,
							   GtkShadowType  shadow,
...

{
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_shadow(style, pixmap, state, shadow, NULL, gtkWidget, part, 0, 0, rect.width(), rect.height()));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
	m_painter->drawPixmap(rect.topLeft(), cache);
...
void QGtk2Painter::paintFlatBox(GtkWidget	  *gtkWidget,
								const gchar	  *part,
								const QRect	  &rect,
								GtkStateType   state,
								GtkShadowType  shadow,
								...
								const QString &pmKey)
{
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_flat_box(style, pixmap, state, shadow, NULL, gtkWidget, part, 0, 0, rect.width(), rect.height()));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
	m_painter->drawPixmap(rect.topLeft(), cache);
								  ...
								  GtkStyle		 *style)
{
	...
	{
		DRAW_TO_CACHE(
			gtk_paint_extension(style, pixmap, state, shadow, NULL, gtkWidget, (const gchar *)part, 0, 0, rect.width(), rect.height(), gap_pos));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
...
void QGtk2Painter::paintOption(GtkWidget	 *gtkWidget,
...

{
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_option(style, pixmap, state, shadow, &gtkCliprect, gtkWidget, detail.toLatin1(), xOffset, yOffset,
...
void QGtk2Painter::paintCheckbox(GtkWidget	   *gtkWidget,
...

{
	...
	if (!m_usePixmapCache || !QPixmapCache::find(pixmapName, &cache))
	{
		DRAW_TO_CACHE(gtk_paint_check(style, pixmap, state, shadow, &gtkCliprect, gtkWidget, detail.toLatin1(), xOffset, yOffset, checkrect.width(),
									  checkrect.height()));
		if (m_usePixmapCache)
			QPixmapCache::insert(pixmapName, cache);
	}
