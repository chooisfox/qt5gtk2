...
{
	connect(parent, &QWindow::destroyed, this, &QGtk2Dialog::onParentWindowDestroyed, Qt::UniqueConnection);
	setParent(parent);
	setFlags(flags);
	...
	if (parent)
	{
		XSetTransientForHint(gdk_x11_drawable_get_xdisplay(gtkWidget->window), gdk_x11_drawable_get_xid(gtkWidget->window), parent->winId());
	}
...
{
	// The QGtk2*DialogHelper classes own this object. Make sure the parent
	// doesn't delete it.
	setParent(0);
}
...
void Qt5Gtk2ColorDialogHelper::applyOptions()
{
	...
	GtkWidget *gtkColorSelection = gtk_color_selection_dialog_get_color_selection(GTK_COLOR_SELECTION_DIALOG(gtkDialog));
	gtk_color_selection_set_has_opacity_control(GTK_COLOR_SELECTION(gtkColorSelection),
...
{
	d.reset(new QGtk2Dialog(gtk_file_chooser_dialog_new("", 0, GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OK,
...
{
	// After GtkFileChooserDialog has been hidden,
	// gtk_file_chooser_get_current_folder() & gtk_file_chooser_get_filenames()
	// will return bogus values -> cache the actual values before hiding the
	// dialog
	_dir	   = directory();
